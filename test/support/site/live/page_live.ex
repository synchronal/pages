defmodule Test.Site.PageLive do
  ...
  def render(assigns) do
    ...
    <main test-page-id={@page_id}>
      ...
      <.link test-role="patch-live-link" patch={~p"/live/show/link"}>Click me to patch directly to a live view</.link>
  ...
  @impl Phoenix.LiveView
  def mount(_params, _session, socket), do: socket |> assign(count: 0, page_id: "live/show") |> ok()

  @impl Phoenix.LiveView
  def handle_params(%{"case" => case}, _session, socket), do: socket |> assign(page_id: "live/show/#{case}") |> noreply()
  def handle_params(_params, _url, socket), do: socket |> noreply()
